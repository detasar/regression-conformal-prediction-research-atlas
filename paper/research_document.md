...
This Research Document reports a neutral empirical study of regression conformal prediction. The study aggregates 145,839 publication-scoped completed rows across 67 datasets, 95 dataset-alpha cells, and 28 conformal-method labels. The document focuses on what the audited experiment observed, what the evidence can support, and which broader readings require separate validation.
...
| Observed pattern | Which methods looked practically useful here? | Under the current coverage criterion, the fixed-GBM CQR pipeline was most frequently selected; Mondrian calibration and CV+ were secondary candidates. The comparison is experiment-scoped; broader method selection or deployment use would require separate validation. | Universal method selection and deployment use would require a separate validation protocol. |
...
The central supported wording is deliberately narrow: Under the current coverage criterion, the fixed-GBM CQR pipeline was most frequently selected; Mondrian calibration and CV+ were secondary candidates. The comparison is experiment-scoped; broader method selection or deployment use would require separate validation. CQR has 56 descriptive coverage-gated selected cells, and CV+ has 13. These counts support a practical-candidate reading within the audited experiment.
...
| Which conformal approaches looked practically useful in the audited experiments? | Under the current coverage criterion, the fixed-GBM CQR pipeline was most frequently selected; Mondrian calibration and CV+ were secondary candidates. The comparison is experiment-scoped; broader method selection or deployment use would require separate validation; CQR has 56 descriptive coverage-gated selected cells and CV+ has 13. | Observed method behavior table, result reading guide, row-weighted coverage summaries, and robustness diagnostics. | Present CQR, CV+, and other methods as experiment-scoped patterns rather than method choices or general deployment rules. |
...
- Under the current coverage criterion, the fixed-GBM CQR pipeline was most frequently selected; Mondrian calibration and CV+ were secondary candidates. The comparison is experiment-scoped; broader method selection or deployment use would require separate validation.
