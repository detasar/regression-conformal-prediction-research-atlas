...
| What does the CQR/CV+ finding mean? | The fixed-GBM CQR pipeline, Mondrian calibration, and CV+ looked practically useful within this experiment, with CQR carried the largest coverage-gated selected-cell signal and a completed backend-sensitivity check. | CQR has 56 coverage-gated selected cells and CV+ has 13 coverage-gated selected cells; the model-matched CQR rerun completed 4,564 rows and 224 paired cells. | Study-wide method choice, universal best-method statements, and deployment use require separate validation. |
...
| Reproducibility and traceability | How can a reviewer trace the evidence? | 3,643 KG nodes, 21,019 KG edges, 0 isolated nodes, and edge selector provenance coverage 1.0000. | The KG and Research Atlas package help readers inspect the evidence trail for this study. |
...
| Empirical result wording | CQR/CV+ are written only as strong practical candidates observed in these experiments. | CQR coverage-gated selected cells 56; CV+ coverage-gated selected cells 13; claim/evidence matrix status pass. | Method-choice, best-method, deployment, and universal-superiority readings require separate validation. |
...
| KG and site publication | The KG and Research Atlas site are useful for review navigation and claim tracing. | 3,643 KG nodes, 21,019 edges, 0 isolated nodes, and edge selector provenance coverage 1.0000. | The KG and GitHub Pages site let readers inspect how claims connect to evidence. |
