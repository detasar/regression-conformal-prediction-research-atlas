...
Which practical method pattern is supported descriptively? & Under the current coverage criterion, the fixed-GBM CQR pipeline was most frequently selected; Mondrian calibration and CV+ were secondary candidates. The comparison is experiment-scoped; using the pattern in a new setting would need its own validation plan. CQR carried the largest coverage-gated selected-cell count (56) and CV+ contributed 13 coverage-gated selected cells. & Coverage-gated selected-cell counts, coverage-width diagnostics, and claim/evidence matrix. & Keep the pattern within experiment-scoped interpretation. \\
...
CQR & Lower and upper quantile models are conformalized with calibration evidence so intervals can adapt to heterogeneous noise. & Its role here is empirical; it does not establish a universal best method, final method choice, or deployment guidance. \\
