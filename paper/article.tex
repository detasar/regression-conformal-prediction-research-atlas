...
This article therefore treats the empirical output as a controlled measurement exercise. The central question is not 'Which method can we promote?' but 'Which patterns are supported by the completed evidence, and which stronger claims remain outside the current evidence?' That framing is important for readers who are new to conformal prediction: the coverage target explains what the interval is trying to guarantee, while the interpretation boundaries explain what this particular experiment does not prove.

\subsection{Guarantee And Claim Boundary Snapshot}

The snapshot below is the main article version of the Research Document's guarantee boundary ledger. It separates conformal prediction's theorem-level language from the empirical audit language used in this study, and it marks readings that remain outside the current evidence in the article.
...
\toprule
Topic & Article statement & Outside current evidence reading \\
\midrule
...
Empirical coverage & Coverage values in this article summarize observed held-out behavior inside the audited dataset-alpha-method scope. & They are not theorem claims and not general product guidance. \\
...
Coverage-gated selection evidence & Coverage-gated selected-cell counts describe observed coverage-width trade-offs among audited methods. & They are not final method selection or universal superiority claims. \\
...
\item It reports CQR/CV+ as strong practical candidates observed in these experiments, without converting that observation into a universal deployment rule.
...
\item It keeps population-level group inference, bounded-support validity, final method selection, public release, and citable web-artifact claims outside current evidence unless their explicit evidence gates are opened later.
\end{itemize}

\subsection{Paper Architecture And Review Contract}

The paper is organized as a small article, a broad supplement, and working navigation artifacts. This structure follows the source-backed publication exemplar review: the article carries the central claim-evidence narrative, while the supplement and KG carry the detailed audit trail. The contract below tells a reader where to look and what each surface must not claim.
...
Claim-evidence contract & Use the claim matrix to keep each reader-facing sentence tied to evidence, citation requirements, and an explicit outside current evidence overclaim. & No prose may convert a outside current evidence claim into a positive conclusion. & claim\_\allowbreak{}boundary\_\allowbreak{}for\_\allowbreak{}every\_\allowbreak{}surface \\
\bottomrule
\end{longtable}
\endgroup
...
The article is organized around five research questions. The answers below are intentionally scoped: each answer states what the audited evidence supports and the stronger reading that remains outside the current evidence.
...
\begin{longtable}{@{}>{\RaggedRight\arraybackslash}p{0.190\linewidth}>{\RaggedRight\arraybackslash}p{0.270\linewidth}>{\RaggedRight\arraybackslash}p{0.270\linewidth}>{\RaggedRight\arraybackslash}p{0.210\linewidth}@{}}
\toprule
Research question & Article answer & Evidence anchor & Outside current evidence reading \\
\midrule
...
Which practical method pattern is supported descriptively? & Within this retrospective and imbalanced experiment surface, the fixed-GBM CQR pipeline was selected most often under the coverage-gated interval-score rule; Mondrian calibration and CV+ were secondary practical candidates; CQR carried the largest coverage-gated selected-cell count (56) and CV+ contributed 13 coverage-gated selected cells. & Coverage-gated selected-cell counts, coverage-width diagnostics, and claim/evidence matrix. & Universal method selection or deployment use would require separate validation. \\
...
Which stronger positive claims remain outside the current evidence? & Bounded-support validity and population-level group inference remain outside the current evidence: 0 bounded-support-validity-ready bundles and 0 population-group-inference-ready bundles. & Paper gate map, bounded-support audit, group-diagnostic audit, and claim matrix. & Keep diagnostic endpoint and group evidence separate from validity or group-inference conclusions. \\
How can the evidence be audited? & The KG and Research Atlas package provide traceability infrastructure with 3,643 KG nodes and 21,019 KG edges. & Knowledge-graph quality audit and sterile package metadata. & Use the KG and site to inspect how claims connect to evidence. \\
\bottomrule
\end{longtable}
\endgroup
...
\toprule
Concept & Source or evidence anchor & Article use & Safe sentence & Outside current evidence sentence \\
\midrule
...
The table below is the draft's control surface: every reader-facing claim is paired with source-backed evidence and an explicit boundary. This keeps useful empirical signals separate from unsupported deployment rules, group-inference claims, endpoint-validity claims, and release claims.
...
\midrule
Audited empirical scope & 145,839 publication-scoped rows, 67 datasets, 95 dataset-alpha cells, and 28 method labels. & The study has a broad audited regression-CP accounting base. & Scope does not by itself authorize external validity or a general deployment guidance. \\
CQR/CV+ descriptive performance & CQR appears in 56 coverage-gated selected cells; CV+ appears in 13 coverage-gated selected cells. & Within this retrospective and imbalanced experiment surface, the fixed-GBM CQR pipeline was selected most often under the coverage-gated interval-score rule; Mondrian calibration and CV+ were secondary practical candidates. & Universal best-method claims require separate validation. \\
...
Knowledge-graph traceability & 3,643 KG nodes and 21,019 KG edges. & The KG is evidence infrastructure linking artifacts, sections, citations, claims, and quality checks. & The KG becomes a citable web artifact only after sterile-repository review and release. \\
\bottomrule
\end{longtable}
\endgroup
...
\begin{longtable}{@{}>{\RaggedRight\arraybackslash}p{0.150\linewidth}>{\RaggedRight\arraybackslash}p{0.220\linewidth}>{\RaggedRight\arraybackslash}p{0.220\linewidth}>{\RaggedRight\arraybackslash}p{0.200\linewidth}>{\RaggedRight\arraybackslash}p{0.150\linewidth}@{}}
\toprule
Stage & Reader question & Evidence object & Allowed claim & Outside current evidence upgrade \\
\midrule
...
Outside current evidence positive gate & Which stronger claims stay absent? & 0 bounded-support-validity-ready bundles and 0 population-group-inference-ready bundles. & Outside current evidence gates are reported as scientific results. & Zero-ready gates cannot be converted into validity or group-inference claims. \\
...
The checklist below is intentionally conservative. It translates the method primer into interpretation scope so a reader can tell which parts are conformal-method background, which parts are empirical findings from this study, and which stronger readings remain outside the current evidence.
...
\begin{longtable}{@{}>{\RaggedRight\arraybackslash}p{0.250\linewidth}>{\RaggedRight\arraybackslash}p{0.340\linewidth}>{\RaggedRight\arraybackslash}p{0.350\linewidth}@{}}
\toprule
Concept & Reader-safe meaning & Outside scope reading \\
\midrule
...
CQR & Lower and upper quantile models are conformalized with calibration evidence so intervals can adapt to heterogeneous noise. & It is not a final selected method, universal best method, or deployment guidance. \\
...
What is CQR? & Conformalized Quantile Regression: quantile models plus conformal calibration. & It is an observed strong practical candidate here, not a universal deployment rule. \\
...
Coverage-gated selected cell & Observed coverage/width trade-off cell & Diagnostic pattern, not final selection \\
...
CQR & Lower/upper quantile models plus conformal calibration & Strong practical candidate observed here; not a universal deployment rule \\
...
The descriptive method result is internally consistent with the individual experiment report: CQR has the largest current coverage-gated selected-cell share, but the final selection claim remains outside current evidence. The correct claim is not that CQR is the best regression conformal method in general; the supported claim is that CQR is the largest current diagnostic coverage-gated selected pattern in this audited study.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{@{}>{\RaggedRight\arraybackslash}p{0.300\linewidth}>{\RaggedRight\arraybackslash}p{0.640\linewidth}@{}}
\toprule
...
The outside current evidence claims are also results. Zero bounded-support validity-ready bundles and zero population-group-inference-ready bundles mean the article should not claim endpoint validity or group inference. Those boundaries prevent useful diagnostics from becoming stronger claims than the evidence supports.

\section{Reproducibility And Traceability}

The knowledge graph snapshot used by this draft has 3,643 nodes and 21,019 edges. The graph links report sections, source artifacts, citation keys, interpretation scope, and quality checks. It is evidence infrastructure for the article and will become citable only after the sterile repository and release review are complete.

\section{Limitations}

\begin{itemize}

\item The method evidence is descriptive and diagnostic; it is not a general deployment rule.
...
This report closes with a descriptive, not prescriptive, interpretation of the completed regression conformal-prediction evidence. The audited evidence base supports reporting the fixed-GBM CQR pipeline, Mondrian calibration, and CV+ as practical candidates within this experiment and the evaluated Venn-Abers regression bridge as bridge-specific negative evidence. It does not support final method selection, broad deployment guidance, bounded-support validity, or population-level group inference.

The scientific contribution is therefore the audited measurement and boundary record itself: useful empirical patterns are retained, negative evidence is not rewritten as success, and outside current evidence claims remain visible until later evidence explicitly opens them.
