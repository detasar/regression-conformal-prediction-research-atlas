...
def test_public_package_data_includes_experiment_configs() -> None:
    ...
    config_paths = sorted(
        path for path in config_root.iterdir() if path.name.endswith((".yaml", ".yml"))
    )
    ...
    legacy_terms = ("frontier", "near_nominal", "near nominal", "near-nominal")
    violations = [
        (path.name, term)
...
def test_public_kg_and_artifact_manifest_are_consistent() -> None:
    ...
    for legacy in (
        ...
        phrase("interpretation", "boundaries"),
...
def test_public_results_page_exposes_interactive_atlas_layers() -> None:
    ...
    for fragment in [
        ...
        "normalized interval-score ratio",
        "Coverage-Width Pareto Summary",
        "CQR Backend Sensitivity Map",
        "Copy Current View",
        "Share filtered result view",
        "Explorer data contracts",
        "../ui_data/dataset_method_heatmap.json",
        "../ui_data/method_family_summary.json",
        "../ui_data/public_research_atlas_manifest.json",
        "Numerical Pathology And Display Policy",
        "numerical_pathology_report.md",
        "Display CI high",
        "Numerical pathology",
...
def test_public_reader_surfaces_avoid_machine_gate_language() -> None:
    ...
    forbidden = [
        ...
        "guard" + "rail",
        "guard" + "rails",
        phrase("claim", "boundary"),
        "claim" + "_boundary",
        "section" + "_claim" + "_boundary" + "_audit",
        phrase("interpretation-boundary"),
        phrase("section-boundary"),
        phrase("README", "navigation", "does", "not", "establish"),
        "rendered_outputs",
        "main_article_review",
        "supplementary_document_review",
        "governance/publication_authoring_decision_record",
        "Research Document/research_document.md",
        phrase("closed", "reading"),
        phrase("not", "a", "method", "recommendation"),
        phrase("method", "recommendation"),
        ...
        phrase("do", "not", "cite"),
        phrase("not", "yet"),
        phrase("Research", "Document-ready"),
        phrase("manuscript-ready"),
        phrase("final-results", "paper"),
        phrase("No", "final", "conformal", "method/model"),
        phrase("selected", "method", "can", "be", "claimed"),
